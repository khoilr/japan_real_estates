...
async def extract_urls_at_page(response_text: str, index: int):
    ...
    for real_estate_div in real_estate_divs:
        ...
        url = real_estate_div.select_one(".heading > a")["href"]
        data["url"] = url
        b_type = real_estate_div.select(".bType").text.strip()
        data["type"] = b_type
        name = real_estate_div.select(".bukkenName").text.strip()
        data["name"] = name

        tr_specs = real_estate_div.select(".bukkenSpec > table > tbody > tr")
        for tr in tr_specs:
            th = tr.select("th")
            td = tr.select("td")

        #     for th, td in zip(th, td):
        #         if th.select('br') and td.select('br'):
